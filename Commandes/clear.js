...
module.exports.run = async(client, message, args) => {
    ...
    if(!args[0]) return message.channel.send(`Vous devez specifiez un nombre de messages à supprimer !`);

    if(isNaN(args[0])) return message.channel.send(`Veuillez spécifiez un nombre !`);

    message.channel.bulkDelete(args[0]);

    message.channel.send(`${args[0]} message on été supprimés !`).then(msg => msg.delete(5000))
